  ...
  changeRoomResolver: async (root, args, ctx) => {
    ...
    try {
      ...
      if (currentRoom.name === "Main" && !currentRoom.comments.length) {
        const prompt = `Human: Let's pretend this is a new chat app about dating where you can create profiles, video chat, share location, and create chatrooms of particular interest; please welcome the user with @${user.username} to "GoneChatting" and tell the slogan, "slogan: where you will never catch a catfish".  let them know this is the hottest new app on the block and to tell their momma and their freinds (something like that), and inform them of some of the features of the app and how it works, let them know it is new and we are tired of all the filters on pictures that people use in other dating apps, that is why here at Gone Chatting we only do Live Video Calls and Video Messages exclusively to communicate! Then ask them a random personal question.`;
        try {
